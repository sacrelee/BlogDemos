...
这个特性通过把表达式自动封装成无参数的闭包来延迟表达式的计算。它可以修饰类型为返回表达式结果类型的无参数函数类型的函数参数。含有 autoclosure 特性的声明同时也具有 noescape 的特性，除非传递可选特性参数 escaping。关于如何使用 autoclosure 特性的例子，请参阅 自动闭包 和 函数类型。
...
在函数或者方法声明上使用该特性，它表示参数将不会被存储以供延迟执行，这将确保参数不会超出函数调用的生命周期。在使用 noescape 声明特性的函数类型中访问属性和方法时不需要显式地使用 self.。
*/
